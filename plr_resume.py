...
def obtener_proximo_movimiento(fullpath, byte_pos):
    try:
        with open(fullpath, 'rb') as f:
            f.seek(byte_pos)
            future_data = f.read(8192).decode('utf-8', errors='ignore')
...
def generar_archivo_recuperacion(original_fullpath, filename, byte_pos, z_pos, estado, metadatos, z_offset):
    ...
    byte_pos_ajustado = byte_pos
    byte_limpio = alinear_byte_a_inicio_linea(original_fullpath, byte_pos_ajustado)

    # Obtener altura de máquina
    linea_altura = buscar_desde_elfinal(original_fullpath, r';\s*printable_height\s*=\s*([0-9.]+)', default = 260)
    
    # Marga de altura extre la boquilla y el eje X (Para impresion por objetos)  
    extruder_clearance = float(re.search(r'(\d+)', buscar_desde_elfinal(original_fullpath, r';\s*extruder_clearance_height_to_rod\s*=\s*(\d+)', default="40")).group(1))
    ...
    with open(recovery_fullpath, 'w', encoding='utf-8') as out:
        ...
        # Movimiento en L
        if target_xy:
            # Ir a la vertical del fallo, pero a 2mm de altura
...
def main():
    ...
    byte_pos, z_pos, z_offset = int(float(raw_byte)), float(raw_z), float(raw_z_offset)
    ...
    try:
        requests.post(MOONRAKER_URL, json={"script": f'_PLR_AUTO_PRINT_RECOVERY FILE="{rec_file}"'})
        print(f"Rescate exitoso: {rec_file}")
    except Exception as e:
        print(f"Archivo generado: {rec_file} (Error Moonraker: {e})")
